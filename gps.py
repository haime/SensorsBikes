...
while True:
    ...
    try:
        ...
        #print time.asctime(time.localtime(x.date))+" Satelites"+str(x.satellites)

        if( noRead < 100 ):
            avgLat = avgLat + x.latitudeDegrees
            avgLon = avgLon + x.longitudeDegrees
            noRead = noRead+1
            print(noRead)
        else:
            print ">>>>>" + str(avgLat /100.0 ) + " , "+ str(avgLon / 100.0)
            avgLat = 0.0
            avgLon = 0.0
            noRead = 0
    except ValueError:
